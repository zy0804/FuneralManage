...
function CremationToExcelCallBack() {
	if (http_request.readyState == 4) {
		if (http_request.status == 200) {
			...
			var myrange=oXL.Range(oSheet.Cells(1,1),oSheet.Cells(1,12));
			myrange.Columns.AutoFit();
			myrange.MergeCells=true;
			
			oSheet.Cells(1,1).value="东台市殡仪馆火化基本信息统计表(三仓分馆)";
			oSheet.Cells(1,1).HorizontalAlignment=3;
			oSheet.Cells(2, 1).NumberFormatLocal = "@";// 将单元格的格式定义为文本
			oSheet.Cells(2, 2).NumberFormatLocal = "@";// 将单元格的格式定义为文本
			oSheet.Cells(2, 3).NumberFormatLocal = "@";// 将单元格的格式定义为文本
			oSheet.Cells(2, 4).NumberFormatLocal = "@";// 将单元格的格式定义为文本
			oSheet.Cells(2, 5).NumberFormatLocal = "@";// 将单元格的格式定义为文本
			oSheet.Cells(2, 6).NumberFormatLocal = "@";// 将单元格的格式定义为文本
			oSheet.Cells(2, 7).NumberFormatLocal = "@";// 将单元格的格式定义为文本
			oSheet.Cells(2, 8).NumberFormatLocal = "@";// 将单元格的格式定义为文本
			oSheet.Cells(2, 9).NumberFormatLocal = "@";// 将单元格的格式定义为文本
			oSheet.Cells(2, 10).NumberFormatLocal = "@";// 将单元格的格式定义为文本
			oSheet.Cells(2, 11).NumberFormatLocal = "@";// 将单元格的格式定义为文本
			oSheet.Cells(2, 12).NumberFormatLocal = "@";// 将单元格的格式定义为文本

			oSheet.Cells(2, 1).value = cremationInfo.rows(1).cells(0).innerText;
			oSheet.Cells(2, 2).value = cremationInfo.rows(1).cells(1).innerText;
			oSheet.Cells(2, 3).value = cremationInfo.rows(1).cells(2).innerText;
			oSheet.Cells(2, 4).value = cremationInfo.rows(1).cells(3).innerText;
			oSheet.Cells(2, 5).value = cremationInfo.rows(1).cells(4).innerText;
			oSheet.Cells(2, 6).value = cremationInfo.rows(1).cells(5).innerText;
			oSheet.Cells(2, 7).value = cremationInfo.rows(1).cells(6).innerText;
			oSheet.Cells(2, 8).value = cremationInfo.rows(1).cells(7).innerText;
			oSheet.Cells(2, 9).value = cremationInfo.rows(1).cells(8).innerText;
			oSheet.Cells(2, 10).value = cremationInfo.rows(1).cells(9).innerText;
			oSheet.Cells(2, 11).value = cremationInfo.rows(1).cells(10).innerText;
			oSheet.Cells(2, 12).value = cremationInfo.rows(1).cells(11).innerText;

			for (var i = 0; i < length; i++) {// 在Excel中写行

				// 定义格式
				oSheet.Cells(i + 3, 1).NumberFormatLocal = "@";// 将单元格的格式定义为文本
				oSheet.Cells(i + 3, 2).NumberFormatLocal = "@";// 将单元格的格式定义为文本
				oSheet.Cells(i + 3, 3).NumberFormatLocal = "@";// 将单元格的格式定义为文本
				oSheet.Cells(i + 3, 4).NumberFormatLocal = "@";// 将单元格的格式定义为文本
				oSheet.Cells(i + 3, 5).NumberFormatLocal = "@";// 将单元格的格式定义为文本
				oSheet.Cells(i + 3, 6).NumberFormatLocal = "@";// 将单元格的格式定义为文本
				oSheet.Cells(i + 3, 7).NumberFormatLocal = "@";// 将单元格的格式定义为文本
				oSheet.Cells(i + 3, 8).NumberFormatLocal = "@";// 将单元格的格式定义为文本
				oSheet.Cells(i + 3, 9).NumberFormatLocal = "@";// 将单元格的格式定义为文本
				oSheet.Cells(i + 3, 10).NumberFormatLocal = "@";// 将单元格的格式定义为文本
				oSheet.Cells(i + 3, 11).NumberFormatLocal = "@";// 将单元格的格式定义为文本
				oSheet.Cells(i + 3, 12).NumberFormatLocal = "@";// 将单元格的格式定义为文本

				var inTime = jsonValue2[i].inTime;
				inTime = inTime.substring(0, 10);

				oSheet.Cells(i + 3, 1).value = inTime;// 向单元格写入值

				oSheet.Cells(i + 3, 2).value = jsonValue2[i].remainsNumber;// 向单元格写入值
				oSheet.Cells(i + 3, 3).value = jsonValue2[i].deadName;// 向单元格写入值
				oSheet.Cells(i + 3, 4).value = jsonValue2[i].deadSex;// 向单元格写入值
				oSheet.Cells(i + 3, 5).value = jsonValue2[i].deadAge;// 向单元格写入值
				oSheet.Cells(i + 3, 6).value = "";// 向单元格写入值
				oSheet.Cells(i + 3, 7).value = jsonValue2[i].address;// 向单元格写入值
				oSheet.Cells(i + 3, 8).value = "'" + jsonValue2[i].deadId;// 向单元格写入值
				var deadTime = jsonValue2[i].deadTime + "";
				deadTime = deadTime.substring(0, 10);
				oSheet.Cells(i + 3, 9).value = deadTime;// 向单元格写入值
				oSheet.Cells(i + 3, 10).value = jsonValue2[i].deadReason;// 向单元格写入值
				oSheet.Cells(i + 3, 11).value = jsonValue2[i].memberMobile;// 向单元格写入值
				oSheet.Cells(i + 3, 12).value = jsonValue2[i].invoiceNo;// 向单元格写入值
...
function BenefitToExcelCallBack() {
	...
	if (http_request.readyState == 4) {
		if (http_request.status == 200) {
			...
			var oWB = oXL.Workbooks.Add();// 新建一个Excel工作簿
			var oSheet = oWB.ActiveSheet;// 指定要写入内容的工作表为活动工作表
			var myrange=oXL.Range(oSheet.Cells(1,1),oSheet.Cells(1,13));
			myrange.MergeCells=true;
			//myrange.
			oSheet.Cells(1,1).value="东台市殡仪馆惠民补助信息统计表(三仓分馆)";
			oSheet.Cells(1,1).HorizontalAlignment=3;
			oSheet.Cells(2, 1).NumberFormatLocal = "@";// 将单元格的格式定义为文本
			oSheet.Cells(2, 2).NumberFormatLocal = "@";// 将单元格的格式定义为文本
			oSheet.Cells(2, 3).NumberFormatLocal = "@";// 将单元格的格式定义为文本
			...
			oSheet.Cells(2, 11).NumberFormatLocal = "@";// 将单元格的格式定义为文本
			oSheet.Cells(2, 12).NumberFormatLocal = "@";// 将单元格的格式定义为文本
			oSheet.Cells(2, 13).NumberFormatLocal = "@";// 将单元格的格式定义为文本

			oSheet.Cells(2, 1).value = benefitInfo.rows(1).cells(0).innerText;
			oSheet.Cells(2, 2).value = benefitInfo.rows(1).cells(1).innerText;
			oSheet.Cells(2, 3).value = benefitInfo.rows(1).cells(2).innerText;
			oSheet.Cells(2, 4).value = benefitInfo.rows(1).cells(3).innerText;
			oSheet.Cells(2, 5).value = benefitInfo.rows(1).cells(4).innerText;
			oSheet.Cells(2, 6).value = benefitInfo.rows(1).cells(5).innerText;
			oSheet.Cells(2, 7).value = benefitInfo.rows(1).cells(6).innerText;
			oSheet.Cells(2, 8).value = benefitInfo.rows(1).cells(7).innerText;
			oSheet.Cells(2, 9).value = benefitInfo.rows(1).cells(8).innerText;
			oSheet.Cells(2, 10).value = benefitInfo.rows(1).cells(9).innerText;
			oSheet.Cells(2, 11).value = benefitInfo.rows(1).cells(10).innerText;
			oSheet.Cells(2, 12).value = benefitInfo.rows(1).cells(11).innerText;
			oSheet.Cells(2, 13).value = benefitInfo.rows(1).cells(12).innerText;
			...
			for (var i = 0; i < length ; i++) {// 在Excel中写行
				...
				oSheet.Cells(i + 3, 11).NumberFormatLocal = "@";// 将单元格的格式定义为文本
				oSheet.Cells(i + 3, 12).NumberFormatLocal = "@";// 将单元格的格式定义为文本
				oSheet.Cells(i + 3, 13).NumberFormatLocal = "@";// 将单元格的格式定义为文本
				var inTime1 = jsonValue2[i].inTime;
				inTime1 = inTime1.substring(0, 10);

				oSheet.Cells(i + 3, 1).value = inTime1;// 向单元格写入值

				oSheet.Cells(i + 3, 2).value = jsonValue2[i].deadName;// 向单元格写入值
				oSheet.Cells(i + 3, 3).value = jsonValue2[i].deadSex;// 向单元格写入值
				oSheet.Cells(i + 3, 4).value = jsonValue2[i].deadAge;// 向单元格写入值

				oSheet.Cells(i + 3, 5).value = jsonValue2[i].address;// 向单元格写入值
				oSheet.Cells(i + 3, 6).value = jsonValue2[i].deadId;// 向单元格写入值
				oSheet.Cells(i + 3, 7).value = jsonValue2[i].subsidyMoney;// 向单元格写入值
				oSheet.Cells(i + 3, 8).value = jsonValue2[i].deadTime
						.substring(0, 10);// 向单元格写入值
				oSheet.Cells(i + 3, 9).value = jsonValue2[i].deadReason;// 向单元格写入值
				oSheet.Cells(i + 3, 10).value = jsonValue2[i].memberMobile;// 向单元格写入值
				oSheet.Cells(i + 3, 11).value = jsonValue2[i].invoiceNo;// 向单元格写入值
				oSheet.Cells(i + 3, 12).value = jsonValue2[i].subsidyNo;// 向单元格写入值
				oSheet.Cells(i + 3, 13).value = inTime1;// 将单元格的格式定义为文本
...
function ExcelById()
{
	...
	for (var i=2; i <= personalInfo.rows.length - 1; i++) {
		...
		var inTime= document.getElementById("personalInfo").rows[i].cells[0].innerHTML;
		var remainsNumber= document.getElementById("personalInfo").rows[i].cells[1].innerHTML;


			
			deadIdArr[num]="{deadId:'"+deadId+"',inTime:'"+inTime+"',remainsNumber:'"+remainsNumber+"'}";
			num++;
		}
...
function clearFormForService() {
	document.form1.servicedeadName.value = "";
