...
function getBreakages(pageNum)
{
	...
	url = "/FuneralManageSystem/queryBreakage!getBreakages";
	url1 = "/FuneralManageSystem/queryBreakage!getPageCount";
	sendRequest("post", url, data, getBreakagesBack);
 ...
 * @param result
 */
function getBreakagesBack(result)
{
	// 清空表格
	deleteTable();
	...
	else
	{
		...
		for (var i = 0; i < data.length; i++)
		{
			...
			td1.innerHTML = data[i].breakageNumber;
			td2.innerHTML = data[i].staffName;
			td3.innerHTML = data[i].breakageDate.substring(0, data[i].breakageDate.length - 5);
...
function getPageCountBack(result)
{
	...
	page.innerHTML = GetPaging(PageNum, TotalPage, "getBreakages");
}
