...
{
	var data = "";       
    var url = "/FuneralManageSystem/addPurchase!getSuppliers";
...
{
	var createDate = document.getElementById("createDate");// 创建日期文本框
	var time = eval("(" + eval("(" + result + ")") + ")");
	createDate.value = time.startTime;
}
...
{
	var data = "";
	var url = "/FuneralManageSystem/addPurchase!getGoodsTypes";
...
function getGoodsNames(obj)
{
	...
	else
	{
		...
		var url = "/FuneralManageSystem/addPurchase!getGoodsNames";
...
function getUnit(obj)
{
	...
	else
	{
		...
		var url = "/FuneralManageSystem/addPurchase!getUnit";
...
function savePurchaseInfo()
{
	...
	if (checkPurchaseInfo())
	{
		if (confirm("确定提交吗？"))
		{
			var data = "supplierName=" + supplierName.value + "&operator=" + operator.value + "&createDate=" + createDate.value + "&memo=" + memo.value
				+ "&purchaseDetails=" + getPurchaseDetails();
			var url = "/FuneralManageSystem/addPurchase!savePurchaseInfo";
