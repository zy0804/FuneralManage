 ...
 * 页面加载时触发该事件
 */
window.parent.document.getElementById("TabbedPanels1").children[0].children[6].onclick = function()
{
 ...
 * 保存结案信息
 */
var saveCloseInfo = function()
{
	var purchaseNumber = document.getElementById("purchaseNumber").value;// 采购编号
	var data = "purchaseNumber=" + purchaseNumber;
	var url = "/FuneralManageSystem/closePurchase!saveCloseInfo";
	if (confirm("确定要结案吗？\n结案后，该采购单将不再收货！"))
	{
		sendRequest("post", url, data, getSaveCloseInfoBack);
	}
	return false;
};
 ...
 * 返回结案结果
 */
var getSaveCloseInfoBack = function(result)
{
