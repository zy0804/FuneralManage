...
function clearTable() {
	...
	for (var i = 2; i < rowsLength; i++) {
		var goodsCheckBoxInput1 = allFuneralGoodsTable.rows[i].cells[4]
				.getElementsByTagName("input");
		var goodsCheckBoxInput2 = allFuneralGoodsTable.rows[i].cells[12]
				.getElementsByTagName("input");
		...
		if (http_request.status == 200) {
			...
			for (var i = 0; i < jsonLength; i++) {
				...
				for (var j = 2; j < rowsLength; j++) {
					...
					//goodName1=goodName1.substring(0,goodName1.length-1);
					//goodName2=goodName2.substring(0,goodName2.length-1);
...
function showSetServiceDetailCallBack() {
	if (http_request.readyState == 4) {
		if (http_request.status == 200) {
			...
			var json = eval("(" + http_request.responseText + ")");
			json = eval("(" + json + ")");
			var cgChoose = document.form3.urnChooseBox;// 火化
			cgChoose.checked=false;
			...
			var cgBeauty = document.form3.makeBeautyBox;// 火化
			cgBeauty.checked=false;
...
function showAllFuneralGoodsCallBack() {   //返回页面的全部丧葬品信息
	if (http_request.readyState == 4) {
		if (http_request.status == 200) {
			var json = eval("(" + http_request.responseText + ")");
			json = eval("(" + json + ")");
			...
			for (var i = 0; i < jsonLength; i++) {
				...
				var funeralGoodsCell1 = row.insertCell();    //第一列丧葬品名称
				var goodsBeCost1 = row.insertCell();		 //第一列丧葬品售价
				var goodsRealCost1 = row.insertCell();		 //第一列丧葬品优惠的钱
				var balanceNumber1=row.insertCell();		 //第一列丧葬品库存
				var addGoodsButton1 = row.insertCell();		 //第一列丧葬品添加按钮
	...
	for (var i = 2; i < len; i++) {
		...
		if (goodsCheckBoxInput2[0] != null) {
			if (goodsCheckBoxInput2[0].checked) {
				...
				var goodsRealCostInput = tableName.rows[i].cells[10]
						.getElementsByTagName("input");
				var goodsChangeCost = goodsRealCostInput[0].value;
				var goodsRealCost = goodsBeCost - goodsChangeCost + "";
				arrFuneralGoods[numbers] = "{goodsName:'" + goodsName
...
function ReadIDCardForService() {
	...
	if (ret == "0") {
//		fillFormForService();
		var pCardNo = CVR_IDCard.CardNo;    //取得身份证号码
		var pName = CVR_IDCard.Name;        //逝者姓名  
		var pAddress = CVR_IDCard.Address;	//逝者家庭住址			
		var pBorn = CVR_IDCard.Born;		//逝者出生年月日
		var pSex = CVR_IDCard.Sex;			//逝者性别
		var nowDate = new Date();
		var currentTime = nowDate.getFullYear();
		var bornDate = pBorn.substr(0, 4);
		var pAge = currentTime - bornDate + 1;
		...
		document.form3.deadAddress.value = pAddress;
		document.form3.deadAge.value = pAge;
		if (pSex == "1") {
			document.form3.deadSex.selectedIndex = 1;
		}
		if (pSex == "0") {
			document.form3.deadSex.selectedIndex = 2;
		}
...
function clearFormForService() {
	...
	document.form3.deadAddress.value = "";
	document.form3.deadAge.value = "";
	document.form3.deadSex.selectedIndex = 0
	}
...
function showData() {
	...
	sendRequest("post", url, data, showDataBack);
	return false;//阻止页面刷新
}
var showDataBack = function(result) 
{	
...
//	alert(goods[0].goodsName);
	...
	form3.memberMobile.value=service[0].memberMobile;
	
	form3.deadName.value=service[0].deadName;
	if(service[0].deadSex!=""){
		for(var i=0;i<document.getElementById("deadSex").options.length;i++)
	    {
	        if(document.getElementById("deadSex").options[i].text == service[0].deadSex)
	        {
	            document.getElementById("deadSex").options[i].selected=true;
	            break;
	        }
	    }
	...
	form3.deadAge.value=service[0].deadAge;
	if(service[0].remainsNumber>0){
		form3.deadNumber.value=service[0].remainsNumber;
	}else{
		form3.deadNumber.value=service[0].remainsOrderNumber;

	}
	
	form3.inTime.value=service[0].inTime;
	form3.dealerId.value=service[0].dealerId;
	form3.dealerName.value=service[0].dealerName;
	form3.directorName.value=service[0].directorName;
	form3.dealerAddress.value=service[0].dealerAddress;
	
	if(service[0].deadResidence!=""){
		for(var i=0;i<document.getElementById("deadResidence").options.length;i++)
	    {
	        if(document.getElementById("deadResidence").options[i].value == service[0].deadResidence)
	        {
	...
	if(service[0].operatorRelation!=""){
		for(var i=0;i<document.getElementById("operatorRelation").options.length;i++)
	    {
	        if(document.getElementById("operatorRelation").options[i].value == service[0].operatorRelation)
	        {
	...
	if(service[0].ashesDisposition!=""){
		for(var i=0;i<document.getElementById("ashesDisposition").options.length;i++)
	    {
	        if(document.getElementById("ashesDisposition").options[i].value == service[0].ashesDisposition)
	        {
	...
	if(service[0].deadReason!=""){
		for(var i=0;i<document.getElementById("deadReason").options.length;i++)
	    {
	        if(document.getElementById("deadReason").options[i].value == service[0].deadReason)
	        {
	...
	if(service[0].deadType!=""){
		for(var i=0;i<document.getElementById("deadKind").options.length;i++)
	    {
	        if(document.getElementById("deadKind").options[i].value == service[0].deadType)
	        {
	...
	if(service[0].pathogeny!=""){
		
		for(var i=0;i<document.getElementById("pathogeny").options.length;i++)
	    {
	        if(document.getElementById("pathogeny").options[i].value == service[0].pathogeny)
	        {
	...
	if(service[0].deadTime=="0001-01-01 00:00:00"){
		form3.deadTime.value="";
	}
	else{
		
		form3.deadTime.value = service[0].deadTime.substring(0,11);
	}
	form3.deadAddress.value=service[0].address;
	form3.deadProveUnit.value = service[0].proofUnit;
	form3.deadExtraInfo.value = service[0].memo;
	if(service[0].invoiceNo==""){
		form3.invoiceNo.value = "0";
	}else{
		form3.invoiceNo.value = service[0].invoiceNo;

	}
	if(service[0].subsidyNo==""){
		form3.subsidyNo.value = "0";
	}else{
		form3.subsidyNo.value = service[0].subsidyNo;

	}
	if(service[0].subsidyMoney==""){
		form3.subsidyMoney.value = "0";
	}else{
		form3.subsidyMoney.value = service[0].subsidyMoney;

	}
	if(service[0].benefitTime=="0001-01-01 00:00:00"){
		form3.benefitTime.value="";
	}
	else{
		
		form3.benefitTime.value = service[0].benefitTime.substring(0,11);
	}
	
	
	if(service[0].carBeCost!=""){
	
	form3.rentCrystalBeCost.value=service[0].beRentCost;
	form3.rentCrystalRealCost.value=service[0].realRentCost;
	form3.lastRentCrystalRealCost.value=service[0].realRentCost;
	
	form3.rentCrystalCarBeCost.value=service[0].carBeCost;
	form3.rentCrystalCarRealCost.value=service[0].carRealCost;
	form3.lastRentCrystalCarRealCost.value=service[0].carRealCost;
	}

	

	for(j=0;j<service.length;j++){
		if(service[j].CremationTypeNo==03){
			...
			for(i=0; i<cg.options.length; i++)
			{
				if (cg.options[i].value ==service[j].itemName) // 动态选择selected默认值
				{
					...
					cg.options[i].selected = true;
		            document.getElementById("cremationBeCost").value=service[j].itemBeCost;
		            
		            document.getElementById("cremationRealCost").value=service[j].itemRealCost;
		...
		if(service[j].CremationTypeNo==02){
			...
			for(i=0; i<cg.options.length; i++)
			{
				...
				if (cg.options[i].value ==service[j].itemName) // 动态选择selected默认值
				{
					...
					cg.options[i].selected = true;
		            document.getElementById("leaveRoomBeCost").value=service[j].itemBeCost;
		            document.getElementById("leaveRoomRealCost").value=service[j].itemRealCost;
		...
		if(service[j].CremationTypeNo==01){
			var cg=document.getElementById("makeBeautyGrade");//火化
			for(i=0; i<cg.options.length; i++)
			{
				if (cg.options[i].value ==service[j].itemName) // 动态选择selected默认值
				{
					...
					cg.options[i].selected = true;
		            document.getElementById("makeBeautyBeCost").value=service[j].itemBeCost;
		            document.getElementById("makeBeautyRealCost").value=service[j].itemRealCost;
	...
	if(service[0].urnName!=""){
		
		var cg=document.getElementById("urnChoose");//火化
		
//		for(i=0; i<cg.options.length; i++)
		for(i=0; i<7; i++)
		{
			
			if (cg.options[i].text ==service[0].urnName) // 动态选择selected默认值
			{
				
				cg.options[i].selected = true;
	            document.getElementById("urnBeCost").value=service[0].urnBeCost;
	            document.getElementById("urnRealCost").value=service[0].urnRealCost;
				document.getElementById("urnRealCost").value=document.getElementById("urnBeCost").value-document.getElementById("urnRealCost").value;					
				document.getElementById("preUrnBeCost").value = Number(service[0].urnBeCost);
				document.getElementById("preUrnRealCost").value = document.getElementById("urnRealCost").value;
				break;
			}
		}
     ...
     var allRealCost=document.getElementById("allRealCost").value;
		...
		var tb=document.getElementById("allFuneralGoods");
...
//					changeGoodsCost2(reduce1[0]);
		...
		var table2=document.getElementById("allFuneralGoods");
...
//				form3.lastWatchSpiritCoffinRealCost.value=json.watchSpiritCoffinRealCost;
				...
				form3.theWholeCost.value=parseInt(form3.theWholeCost.value)-parseInt(form3.lastWholeCost.value);
...
//				form3.allBeCost.value=parseInt(form3.theWholeCost.value)+parseInt(form3.remainsCarryBeCost.value)+parseInt(form3.rentCrystalBeCost.value)+parseInt(form3.rentCrystalCarBeCost.value)+parseInt(form3.watchSpiritVillaBeCost.value)+parseInt(form3.watchSpiritCoffinBeCost.value)-parseInt(json.remainsCarryRealCost)-parseInt(json.rentCrystalRealCost)-parseInt(json.rentCrystalCarRealCost)-parseInt(json.watchSpiritVillaRealCost)-parseInt(json.watchSpiritCoffinRealCost);
				form3.allBeCost.value=parseInt(form3.theWholeCost.value);
...
//				document.getElementById("itemPrice3").innerHTML = "";
				...
				if ((setServiceCheckBox.checked == true) && (setService != "-请选择-")) {
					document.getElementById("setItem1").innerHTML = "<font style='font-weight:bold;'>"
							+ setService.toString().substring(0,setService.length-1) + "</font>";
				}
...
//						goodsCheckBoxInput1[0].onchange();
				//		goodsCheckBoxInput1[0].disabled = "true";
						...
						var goodsCost=Number(allFuneralGoodsTable.rows[j].cells[1].getElementsByTagName("input")[0].value)-Number(allFuneralGoodsTable.rows[j].cells[2].getElementsByTagName("input")[0].value)
						goodsCost=goodsCost+"";
						setServiceDetailContent.value = setServiceDetailContent.value
						+ allFuneralGoodsTable.rows[j].cells[0].innerText+":"+goodsCost+";";
...
//						goodsCheckBoxInput2[0].onchange();
				//		goodsCheckBoxInput2[0].disabled = "true";
						var goodsCost1=Number(allFuneralGoodsTable.rows[j].cells[9].getElementsByTagName("input")[0].value)-Number(allFuneralGoodsTable.rows[j].cells[10].getElementsByTagName("input")[0].value)
							goodsCost1 = goodsCost1+"";
						setServiceDetailContent.value = setServiceDetailContent.value
						+ allFuneralGoodsTable.rows[j].cells[8].innerText+":"+goodsCost1+";";
						FunneralGoodsCost = parseInt(FunneralGoodsCost)
			...
			//alert(setServiceDetailContent.value);
			var leaveRoomBeCost = document.form3.leaveRoomBeCost.value;
			var cremationBeCost = document.form3.cremationBeCost.value;
...
//			var serviceContent = document.form3.serviceContent;
			if ((urnChooseBox.checked == true) && (urnChoose != "-请选择-")) {
				var serviceContent = document.getElementById("serviceContent");
				oneRow = serviceContent.insertRow();//插入一行  
				cell1= oneRow.insertCell();//单单插入一行是不管用的，需要插入单元格  
				cell1.innerHTML = "<font style='font-weight:bold;'>"
					+ urnChoose + "</font>";
				cell2= oneRow.insertCell();//单单插入一行是不管用的，需要插入单元格  
				cell2.innerHTML = "<font style='font-weight:bold;'>"
					+ urnCost + "</font>";
				cell3= oneRow.insertCell();//单单插入一行是不管用的，需要插入单元格  
				cell3.colSpan="8";
				cell3.innerHTML = "";
...
//						+ urnCost + "</font>";
			...
			var makeBeautyBox = document.form3.makeBeautyBox;
			var makeBeautyGrade = document.form3.makeBeautyGrade;
			var makeBeautyIndex = makeBeautyGrade.selectedIndex;
			var makeBeautyChoose = makeBeautyGrade.options[makeBeautyIndex].text;
			var makeBeautyBeCost = document.form3.makeBeautyBeCost.value;
			var makeBeautyRealCost = document.form3.makeBeautyRealCost.value;
			var makeBeautyCost = parseInt(makeBeautyBeCost) - parseInt(makeBeautyRealCost);
			if ((makeBeautyBox.checked == true) && (makeBeautyChoose != "-请选择-")) {
				var serviceContent = document.getElementById("serviceContent");
				oneRow1 = serviceContent.insertRow();//插入一行  
				cell4= oneRow1.insertCell();//单单插入一行是不管用的，需要插入单元格  
				cell4.innerHTML = "<font style='font-weight:bold;'>"
					+ makeBeautyChoose + "</font>";
				cell5= oneRow1.insertCell();//单单插入一行是不管用的，需要插入单元格  
				cell5.innerHTML = "<font style='font-weight:bold;'>"
					+ makeBeautyCost + "</font>";
				cell6= oneRow1.insertCell();//单单插入一行是不管用的，需要插入单元格 
				cell6.colSpan="8";
				cell6.innerHTML = "";
...
function decideInvoice() {
	...
	var inTime = document.form3.inTime.value;
	var deadNumber = document.form3.deadNumber.value;

	
	var deadSexSelect = document.form3.deadSex;
	var deadSexIndex = deadSexSelect.selectedIndex;
	var deadSex = deadSexSelect.options[deadSexIndex].text;
...
//	}
	...
	if(deadName!==""){
		...
		alert("请输入逝者姓名");
		form3.deadName.focus();
		return false;
	}
	url = url + "deadInfo.inTime=" + inTime + "&";
	url = url + "deadInfo.deadNumber=" + deadNumber + "&";

	if (deadSex != "请选择...") {
		url = url + "deadInfo.deadSex=" + deadSex + "&"; // 户籍所在地
	} else {
		alert("请选择逝者性别");
		deadSexSelect.focus();
		return false;
	...
	url = url + "deadInfo.invoiceNo=" + invoiceNo + "&";
	 
	if(benefitTime!=""){
		url = url + "deadInfo.benefitTime=" + benefitTime + "&";
	}else{
		benefitTime="0001-01-01 00:00:00";
		url = url + "deadInfo.benefitTime=" + benefitTime + "&";
	}
	
	url = url + "deadInfo.subsidyNo=" + subsidyNo + "&";
	...
	if (deadExtraInfo != "") {
		url = url + "deadInfo.deadExtraInfo=" + deadExtraInfo + "&"; // 骨灰去向
	} else {
		alert("请输入备注");
		document.form3.deadExtraInfo.focus();
		return false;
	}

	if (directorName != "") {
		url = url + "deadInfo.directorName=" + directorName + "&"; // 骨灰去向
	} else {
		alert("请输入承办人姓名");
		document.form3.directorName.focus();
		return false;
	}
	url = url + "deadInfo.subsidyMoney=" + subsidyMoney;
	http_request = createHttpRequest();
...
function setDetailPrint() {
	...
	var setDetail =  leaveRoomGrade + ":"
	+ leaveRoomCost + ";" + cremationGrade + ":" + cremationCost+";"+setServiceDetailContent;
	// LODOP.ADD_PRINT_TEXT('27mm','55mm',500,20,setDetail);
	LODOP.ADD_PRINT_HTM('30mm', '83mm', 500, 20,                    //调套餐详情打印位置，第一个坐标是上边距，第二个是左边距
			"<font style='font-size:8px;'>" + setDetail + "</font>");
}
...
function CreateCremationPrintCallBack() {
	...
	if (http_request.readyState == 4) {
		if (http_request.status == 200) {
			...
			// LODOP.ADD_PRINT_SETUP_BKIMG("<img border='1'
			// src='http://192.168.1.102:8080/FuneralManageSystem/Images/tax.jpg'/>");
			LODOP.ADD_PRINT_SETUP_BKIMG("<img border='1' src='http://localhost:8080/FuneralManageSystem/Images/tax.jpg'/>");
...
//			var deadTime = deadTime1.substr(0, deadTime1.length - 2);
			...
			var deadTimeDay = deadTime1.substr(8, 2);
...
//	var sumBeCost =Number(remainsCarryBeCost)+Number(rentCrystalBeCost)+Number(rentCrystalCarBeCost)+Number(leaveRoomBeCost)+Number(urnBeCost)+Number(makeBeautyBeCost)+Number(cremationBeCost);
	var sumBeCost =Number(leaveRoomBeCost)+Number(urnBeCost)+Number(makeBeautyBeCost)+Number(cremationBeCost);

//	var sumRealCost = Number(remainsCarryBeCost)-Number(remainsCarryRealCost)+rentCrystalCostChange+rentCrystalCarCostChange+Number(leaveRoomRealCost)+Number(urnRealCost)+Number(makeBeautyRealCost)+Number(cremationRealCost);
	var sumRealCost = Number(leaveRoomRealCost)+Number(urnRealCost)+Number(makeBeautyRealCost)+Number(cremationRealCost);
...
function getDeadNumberCallBack(){
	if (http_request.readyState == 4) {
		if (http_request.status == 200) {
			...
			var json = eval("(" + result + ")");
				json = eval("(" + json + ")");
			document.getElementById("deadNumber").value=json[0].deadNumber;
