...
public class CremationToExcelAction extends ActionSupport{
	...
	public String ExcelById() throws Exception{
		...
		if(allDeadId.length()>1){
			...
			System.out.println(funeralGoodsStr);
			...
			if(jsonArrayIdArray.size()>0){
				for(int i=0;i<jsonArrayIdArray.size();i++){
					...
					remainsNumber=personalId.getInt("remainsNumber");
					inTime=personalId.getString("inTime");
//					System.out.println(remainsNumber);

					returnString= CremationToExcelDao.ExcelById(deadId,inTime,remainsNumber);		
				}
			}
		}
