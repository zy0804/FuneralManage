...
public class CremationToExcelService extends BaseService {
	...
	public String ExcelById(String deadId,String inTime, int remainsNumber) throws SQLException, Exception {
		Connection conn = DBDao.openDateBase("dongtai");
		if (conn != null) {
			...
			for(int i=0,len=Time.length;i<len;i++){
				timeN=timeN+Time[i].toString();
			}
			String fileName = timeN+"-"+num +".xls";

			// File file2 = new File("D:/逝者详单信息Excel表");
			// if (file2.mkdir()) {
			// System.out.println("单文件夹 逝者详单信息Excel表 创建成功！创建后的文件目录为：" +
			// file2.getPath() + ",上级文件为:" + file2.getParent());
			// }
			// CopyFile.copyFile("C:/Users/zy/Desktop/模版.xls","D:/逝者详单信息Excel表/"+fileName);
			// 表格要导出的目录
			String outPath = "F:\\逝者详单信息Excel表\\" + fileName;
			FileOutputStream is = new FileOutputStream(outPath);
			// 模板文件路径
			File sourceXlsFile = new File("F:\\逝者详单信息Excel表\\模版.xls");
			// 读取模板文件
			...
			try {
				...
				while (rs1.next()) {
					...
					if(rs1.getString("dealerId")!=null){
					if(rs1.getString("dealerId").length()>0){
						String dealerYear1 = rs1.getString("dealerId").substring(6,
								10);
						...
						label = new Label(5, 3, rs1.getString("dealerId")
									.substring(6, 14));
						sheet.addCell(label);
					...
					label = new Label(5, 2, rs1.getString("deadId").substring(
							6, 14));
					sheet.addCell(label);
					...
					sheet.addCell(label);
					System.out.println(rs1.getString("deadResidence"));
					label = new Label(2, 6, rs1.getString("deadResidence"));
					sheet.addCell(label);
						...
						for (int k = 0; k < 12; k++) {
							...
							if (cell.getContents() != "") {
								if (value.equals(cell.getContents().toString())) {
									label = new Label(k + 2, j,
											rs3.getString("goodsBeCost"));
									sheet.addCell(label);
									if (temp3 > 0) {
				...
				while (rs4.next()) {
					...
					String itemName = getTypeNumber(cremationTypeNo,
							cremationItemNo);
