...
public class RegisterServiceService extends BaseService {// 登记服务页面显示初始化信息
		...
		if (conn != null) {
			...
			try {
				...
				if (rs.getRow() == 1) {
					...
					String remainsCarryBeCost = getAllServiceDao
							.getRemainsCarryCost(deadId) + ""; // 遗体接运用车费用
																// sum(carBeCost)
					String remainsCarryRealCost = getAllServiceDao
							.getRemainsCarryRealCost(deadId) + "";

					String rentCrystalBeCost = getAllServiceDao
							.getRentCoffinCost(deadId) + ""; // 租棺部分用棺费用
																// sum(beRentCost)
					String rentCrystalRealCost = getAllServiceDao
							.getRentCoffinRealCost(deadId) + "";

					String rentCrystalCarBeCost = getAllServiceDao
							.getRentCrystalCarCost(deadId) + ""; // 租棺部分用车费用
																	// sum(carBeCost)
					String rentCrystalCarRealCost = getAllServiceDao
							.getRentCrystalCarRealCost(deadId) + "";

					String watchSpiritVillaBeCost = getAllServiceDao
							.getWatchSpiritCost(deadId) + ""; // 守灵部分别墅费用
																// sum(villaBeCost)
					String watchSpiritVillaRealCost = getAllServiceDao
							.getWatchSpiritRealCost(deadId) + "";

					String watchSpiritCoffinBeCost = getAllServiceDao
							.getCoffinBeCost(deadId) + ""; // 别墅内部棺材费用
															// sum(coffinBeCost)
					String watchSpiritCoffinRealCost = getAllServiceDao
							.getCoffinRealCost(deadId) + "";
					this.returnString = "{\"deadNumber\":\"" + deadNumber
							+ "\", \"benefitTime\":\"" + benefitTime
							+ "\", \"ashesDisposition\":\"" + ashesDisposition
							+ "\", \"deadResidence\":\"" + deadResidence
							+ "\", \"operatorRelation\":\"" + operatorRelation
							+ "\", \"pathogeny\":\"" + pathogeny
							+ "\", \"watchSpiritCoffinRealCost\":\""
							+ watchSpiritCoffinRealCost
							...
							+ "\", \"rentCrystalRealCost\":\""
							+ rentCrystalRealCost
							+ "\", \"remainsCarryRealCost\":\""
							+ remainsCarryRealCost
							+ "\", \"watchSpiritCoffinBeCost\":\""
							+ watchSpiritCoffinBeCost
							+ "\", \"rentCrystalCarBeCost\":\""
							+ rentCrystalCarBeCost + "\", \"deadSex\":\""
							+ deadSex + "\", \"deadAge\":\"" + deadAge
							+ "\", \"rentCrystalBeCost\":\""
							+ rentCrystalBeCost
							+ "\", \"watchSpiritVillaBeCost\":\""
							+ watchSpiritVillaBeCost
							+ "\", \"remainsCarryBeCost\":\""
							+ remainsCarryBeCost + "\", \"memberMobile\":\""
							+ memberMobile + "\", \"deadName\":\"" + deadName
							+ "\", \"inTime\":\"" + inTime
							...
							+ "\", \"deadTime\":\"" + deadTime
							+ "\", \"deadType\":\"" + rs.getString("deadType")
							+ "\", \"deadReason\":\""
							+ rs.getString("deadReason") + "\", \"address\":\""
							+ rs.getString("address") + "\", \"area\":\""
							+ rs.getString("area") + "\", \"proofUnit\":\""
							+ rs.getString("proofUnit") + "\", \"memo\":\""
							+ rs.getString("memo") + "\", \"invoiceNo\":\""
							+ rs.getString("invoiceNo")
							+ "\", \"subsidyNo\":\""
							+ rs.getString("subsidyNo")
							+ "\", \"subsidyMoney\":\""
							+ rs.getInt("subsidyMoney") + "\", \"flag\":\""
							+ flag + "\"";
				...
				if (conn != null) {
					...
					try {
						...
						while (rs2.next()) {
							this.returnString = returnString + ",\"A"
									+ rs2.getString("cremationTypeNo")
									+ "\":\""
									+ rs2.getString("cremationItemNo") + "\"";
						}
						rs2.close();
						ps2.close();
	...
	public String showAllFuneralGoodsDao() {
		Connection conn = DBDao.openDateBase("dongtai");
		if (conn != null) {
			String sql = "SELECT funeralgoods.goodsName,funeralgoods.goodsBeCost,warehousebalance.balanceNumber FROM  funeralgoods LEFT JOIN warehousebalance ON funeralgoods.`goodsName` = warehousebalance.`goodsName` order by funeralgoods.goodsId asc";
			...
			try {
				...
				if (rs.getRow() == 1) {
					...
					String farewellRoomPrice = rs.getInt("farewellRoomPrice")
							+ "";
					returnString = "{\"furnace\":\"" + furnace
							+ "\", \"furnacePrice\":\"" + furnacePrice
							+ "\", \"farewellRoom\":\"" + farewellRoom
							+ "\", \"farewellRoomPrice\":\""
							+ farewellRoomPrice + "\"}";
				} else {
					this.returnString = "";
